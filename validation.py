...
def render_validation_dashboard(api_key=None, provider=None, model_id=None):
    ...
    predicted = [e.get("risk_level") for e in entries if e.get("risk_level")]
    if predicted:
        eval_res = evaluate_classification(predicted)
        if eval_res["accuracy"] is not None:
            st.metric("Classification Accuracy", f"{eval_res['accuracy']}%",
                      delta=f"{eval_res['correct']}/{eval_res['n']} ถูกต้อง")

    st.markdown("**Test Cases (Ground Truth):**")
    if HAS_PLOT:
        gt_rows = []
        for gt in GROUND_TRUTH_CASES:
            gt_rows.append({
                "Test ID":     gt["test_id"],
                "คำอธิบาย":    gt["description"],
                "Expected Risk": gt["expected_risk"],
            })
        st.dataframe(pd.DataFrame(gt_rows), use_container_width=True, hide_index=True)
    else:
        for gt in GROUND_TRUTH_CASES:
            st.markdown(f"- **{gt['test_id']}** {gt['description']} → expected: `{gt['expected_risk']}`")
