...
with main_tab:
    ...
    with left_col:
        ...
        if "manual_text_key" not in st.session_state:
            st.session_state.manual_text_key = 0

        if "uploaded_image_data" not in st.session_state:
            st.session_state.uploaded_image_data = None

        if "input_mode" not in st.session_state:
            st.session_state.input_mode = "📷 อัปโหลดภาพ"

        # ─────────────────────────────────────────────
        # callback : เคลียร์ input ของฝั่งตรงข้ามเมื่อสลับโหมด
        # ─────────────────────────────────────────────
        def _on_input_mode_change():
            new_mode = st.session_state.input_mode_radio
            if new_mode == "📷 อัปโหลดภาพ":
                # เข้าโหมดรูป → ล้าง text
                st.session_state.manual_text_key += 1   # reset text_area widget
            else:
                # เข้าโหมดพิมพ์ → ล้างรูป
                st.session_state.uploaded_image_data = None
                st.session_state.uploaded_file_key += 1  # reset file_uploader widget
            st.session_state.input_mode = new_mode

        # ─────────────────────────────────────────────
        # input mode selector (แทน st.tabs)
        # ─────────────────────────────────────────────
        st.radio(
            "เลือกวิธีกรอกข้อมูล",
            options=["📷 อัปโหลดภาพ", "✏️ พิมพ์ค่าตรวจ"],
            key="input_mode_radio",
            index=0 if st.session_state.input_mode == "📷 อัปโหลดภาพ" else 1,
            horizontal=True,
            on_change=_on_input_mode_change,
        ...
        # MODE 1 : upload image
        # ─────────────────────────────────────────────
        if st.session_state.input_mode_radio == "📷 อัปโหลดภาพ":
        ...
        # ─────────────────────────────────────────────
            ...
            manual_text = st.text_area(
                "ค่าตรวจ",
                key=f"manual_lab_text_{st.session_state.manual_text_key}",
                height=180,
                label_visibility="collapsed",
                placeholder=(
                    "HbA1c: 6.8\n"
    ...
    if analyze_btn:
        ...
        if not api_key:
            st.error("⚠️ กรุณาใส่ API Key ก่อนใช้งาน")
            st.stop()
        ...
        if raw_text and raw_text != "[image input — OCR via Vision AI]":
            with st.spinner("🤖 NER กำลังระบุค่าตรวจ (BioBERT)..."):
        ...
        SYSTEM_PROMPT = """
                        ...
                        3. อธิบายผลเป็นภาษาไทยที่เข้าใจง่าย **(สำคัญมาก: หากผู้ใช้ระบุ "อาการ" หรือ "โรคประจำตัว" มาด้วย คุณต้องอธิบายเชื่อมโยงด้วยว่าอาการเหล่านั้น เช่น เหนื่อยง่าย ปวดหัวบ่อย มีสาเหตุสัมพันธ์กับค่าแล็บที่ผิดปกติอย่างไร)**
                        4. ให้คำแนะนำการดูแลสุขภาพเบื้องต้นที่สอดคล้องกับผลแล็บและอาการ
                        ...
                        - ในช่อง "abnormal_values" คุณต้องลิสต์ค่าตรวจ "ทั้งหมด" ที่ผู้ใช้ส่งมาและพบว่าผิดปกติ (สูงหรือต่ำกว่าเกณฑ์) ห้ามตัดทอน ห้ามสรุปย่อเด็ดขาด ไม่ว่าจะส่งมากี่ค่าก็ตาม
        ...
        st.session_state.manual_text_key += 1
